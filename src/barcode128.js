...
function createElement(input){
    ...
    if (/^[0-9]{1}$/.test(input)) {
        ...
        elements = chars.map(function (item) {
            var r = null;
            ELEMENT_TABLE.forEach(function (o) {
                if (o[1] === item) r = o;;
            })
            return r;
        ...
        var r = null;
        ELEMENT_TABLE.forEach(function (o) {
            if (o[1] === chars[chars.length - 1]) r = o;;
        })
        elements.push(r);
...
function drawUnit(context, i, uw, uh, color,x,y) {
    context.save();
    try {
        context.beginPath();
		var paddingLeft = x||0;
		var paddingTop = y||0;
        context.rect(uw * i+paddingLeft, paddingTop, uw, uh);
        if (color) {
...
function code128Auto (canvas, input,options) {
	var elements = createElement(input)
	
	var units = elements.map(function (item) {
		return item[5];
	}).join('').split('');
	if (units.length==0) return ;
	var uw = options.unitWidth || (options.width/units.length) || 1;
	var uh = options.height || (uw * 30); //uw * 30;
	var canvasHeight = uh;
	var canvasWidth = units.length * uw;
	if ('string'==typeof canvas){
		if (canvas == "") {
			canvas = document.createElement("canvas"); //.getElementById('tmpcanvas');
			document.body.appendChild(canvas);
			if (options.showText){ canvasHeight = uh+20;}
	...
	if (options.showText){
		...
		//context.fillText(input, 20, uh+10);
		//context.rect(0,uh,canvasWidth,20);
	...
	for (var i = 0, len = units.length; i < len; i++) {
		if (+units[i]) {
			drawUnit(context, i, uw, uh, options.color||'#000',x,y);
		} else {
			drawUnit(context, i, uw, uh, '#fff',x,y);
		}
	}
